...
public class Invader: SKSpriteNode
{
    ...
    {
        let bullet = InvaderLaser(imageName: "laser", bulletSound: "laser sound.mp3")
        bullet.position.x = self.position.x
        bullet.position.y = self.position.y - self.size.height / 2
        scene.addChild(bullet)
        let moveBulletAction = SKAction.move(to: CGPoint (x: self.position.y, y: 0 - bullet.size.height), duration: 2.0)
